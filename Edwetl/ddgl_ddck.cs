...
namespace Edwetl
{
    public partial class ddgl_ddck : Form
    {
        public ddgl_ddck()
        {
            InitializeComponent();
        ...
        public void initp()
        {
            ...
            oporacle db = new oporacle(str_conn);
       ...
       '' sql_str,
       des_tab_name
  from ETL_TAB_MAPPING_DEF
union all
select distinct substr(batch_id, 1, 6) as mapping_id,
                substr(batch_id, 1, 2) as batch_id,
                '' delete_sql,
                '' sql_str,
                des_tab_name
  from ETL_TAB_MAPPING_DEF
   where mapping_id in (
  select max(mapping_id) from ETL_TAB_MAPPING_DEF group by batch_id)
union all  
  select distinct substr(batch_id, 1, 2) as mapping_id,
                substr(batch_id, 1, 1) as batch_id,
                '' delete_sql,
                '' sql_str,
                '' des_tab_name
  from ETL_TAB_MAPPING_DEF
  union all  
  select distinct substr(batch_id, 1, 1) as mapping_id,
                substr('0', 1, 1) as batch_id,
                '' delete_sql,
                '' sql_str,
                '' des_tab_name
  from ETL_TAB_MAPPING_DEF
 order by mapping_id) t where t.mapping_id<>batch_id";
                rtb_log.Text += sql2+"\n";
        ...
        public void initdgv(string str_pid)
        {
            ...
            sb.Append("select mapping_id,batch_id,src_tab_name,des_tab_name,delete_sql,sql_str from ETL_TAB_MAPPING_DEF ");
            sb.Append(" where ");
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            string sql = "update etl_tab_mapping_def set delete_sql='"+str_delete+"',sql_str=:clob where mapping_id='" + str_recid + "'";
        ...
        {
            dbcommon db = new dbcommon();
            richTextBox1.Text = db.get_conn("2");
            //richTextBox1.Text = "Data Source=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=103.160.120.173)(PORT=1521)))(CONNECT_DATA=(SERVICE_NAME=cmm2)));User ID=cust_dm;Password=cust_dm";
            db = null;
        }
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show(result);
            richTextBox3.Text = sql;
            //deal dl = new deal(str_conn, str_date_8, str_del, sql);
            //System.Threading.ThreadStart ts = new System.Threading.ThreadStart(dl.worker);
